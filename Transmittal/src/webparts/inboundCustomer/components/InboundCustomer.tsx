...
import {
  IEmecInboundCustomerWpState,
  IInboundCustomerProps,
  ITableData2,
  projectData,
} from "./IInboundCustomerProps";
import { Web } from "@pnp/sp/webs";
import { BaseService } from "../services/BaseService";

export default class EmecInboundCustomerWp extends React.Component<
  IInboundCustomerProps,
  IEmecInboundCustomerWpState,
  {}
> {
  ...
  constructor(props: IInboundCustomerProps) {
    ...
    this.bindInboundTransmittalSavedData =
      this.bindInboundTransmittalSavedData.bind(this);
    ...
    this._LAUrlGettingForPermission =
      this._LAUrlGettingForPermission.bind(this);
    this.triggerProjectPermissionFlow =
      this.triggerProjectPermissionFlow.bind(this);
    this._LAUrlGettingForDocumentLibraryUpdate =
      this._LAUrlGettingForDocumentLibraryUpdate.bind(this);
  ...
  private _LAUrlGettingForDocumentLibraryUpdate = async () => {
    ...
    const laUrl = await this._Service.getLAurl(
      hubSite,
      this.props.requestList,
      "EMEC_InboundTransmittal"
    );
    ...
    if (response.ok) {
      ...
      if (responseJSON["Status"] == "Valid") {
        ...
        this.setState({
          ...
          statusMessage: {
            isShowMessage: true,
            message: "You are not permitted to perform this operations",
            messageType: 4,
          },
        });
        setTimeout(() => {
  ...
  public async DocIndex(option: { key: any; text: any }) {
    ...
    let select =
      "DocumentID,DocumentName,Owner/ID,Owner/Title,Owner/EMail,Revision,SourceDocument,CriticalDocument,CustomerDocumentNo";
    let expand = "Owner";
    let filter = "ID eq '" + option.key + "'";
    const documentIndexItem: any = await this._Service.getItemWithFilterExpand(
      this.props.siteUrl,
  ...
  public Addindex() {
    if (
      ...
      this.state.outlookPONumber == "" ||
      (this.state.comments == "" && this.state.Attachments == null)
    ) {
  ...
  private async _accessGroups() {
    ...
    accessGroup = await this._Service.getItemWithSelectFilter(
      this.props.siteUrl,
  ...
  private async _gettingGroupID(AccessGroupItems) {
    ...
    for (let a = 0; a < AccessGroupItems.length; a++) {
      ...
      const accessGroupID: any = await this._Service.getItemWithFilter(
        this.props.siteUrl,
  ...
  public AddDoc() {
    ...
    let data: projectData = {
      ...
      Attachments:
        this.state.Attachments == null ? null : this.state.Attachments,
    ...
    let dupIndex = tableData.filter(
      ({ documentIndex }) => documentIndex === this.state.docKey
    );
    ...
    (document.querySelector("#studyAttachment") as HTMLInputElement).value =
      null;
  }
  public AddDoc2() {
    this.setState({ AddIndex2: false });
    ...
    let data2: ITableData2 = {
      documentName2:
        this.state.transmittalID + this.fileInput.current.files[0].name,
      ...
      Attachments2:
        this.state.Attachments2 == null ? null : this.state.Attachments2,
    ...
    (document.querySelector("#additionalfile") as HTMLInputElement).value =
      null;
  }
  public _CompanyDocChange = (
    ev: React.FormEvent<HTMLInputElement>,
    CompanyDocNo?: any
  ) => {
    this.setState({ outlookCustomerDocNo: CompanyDocNo });
  };
  public _PoNumbChange = (
    ev: React.FormEvent<HTMLInputElement>,
    ponumber?: any
  ) => {
  ...
  private handleFileUpload = (event) => {
    ...
    this.file2 = (
      document.getElementById("studyAttachment") as HTMLInputElement
    ).files[0];
    this.setState({
  ...
  private handleFileUpload2 = (event) => {
    let file3 = (document.getElementById("additionalfile") as HTMLInputElement)
      .files[0];
    this.setState({
      Attachments2: file3,
    });
  ...
  //send mail
  public _sendmail = async (
    docid,
    emailuser,
    type,
    name,
    TransmittalAcceptanceCode
  ) => {
    ...
    let Subject;
    let Body;
    const notificationPreference: any[] = await this.reqWeb
      .getList(
        "/sites/" +
          this.props.hubsite +
          "/Lists/" +
      ...
      .items.filter("EmailUser/EMail eq '" + emailuser + "'")
      .select("Preference")
      .get();
    if (notificationPreference.length > 0) {
      if (notificationPreference[0].Preference == "Send all emails") {
        ...
        notificationPreference[0].Preference ==
        "Send mail for critical document"
      ) {
        mailSend = "Yes";
      } else {
        mailSend = "Yes";
    ...
    if (mailSend == "Yes") {
      const emailNotification: any[] = await this.reqWeb
        .getList(
          "/sites/" +
            this.props.hubsite +
            "/Lists/" +
      ...
      let replaceBody = replaceString(
        replaceRequester,
        "[DocumentName]",
        docid
      );
      let finalBody = replaceString(
        replaceBody,
        "[TransmittalAcceptanceCode]",
        TransmittalAcceptanceCode
      );
  ...
  public async _trannsmittalIDGeneration() {
    ...
    await this._Service
      .getItemWithSelect(
        this.props.siteUrl,
        this.props.TransmittalIDSettings,
        select
      )
  ...
  protected async _triggerPermission(SourceDocutID) {
    const laUrl = await this.reqWeb
      .getList(
        "/sites/" + this.props.hubsite + "/Lists/" + this.props.requestList
      )
  ...
  public async submit() {
    if (this.state.ReactTableResult.length == 0) {
      this.setState({
        statusMessage: {
          isShowMessage: true,
          message: "Please add atleast one project document ",
          messageType: 1,
        },
      });
      setTimeout(() => {
      ...
      if (this.transmittalID == null || this.transmittalID == "") {
        ...
        this._Service
          .createNewItem(
            this.props.siteUrl,
            this.props.InboundTransmittalHeader,
            inboundHeader
          )
          .then(async (inboundTransmittalHeader) => {
            this.setState({
              inboundTransmittalHeaderId: inboundTransmittalHeader.data.ID,
            });
            ...
            let inboundLinks = {
              TransmittalDetails: {
                ...
                Url:
                  this.props.siteUrl +
                  "/Lists/" +
                  "InboundTransmittalDetails" +
                  "/AllItems.aspx?FilterField1=TransmittalHeader&FilterValue1=" +
                  inboundTransmittalHeader.data.ID +
              ...
              InboundAdditionalDetails: {
                ...
                Url:
                  this.props.siteUrl +
                  "/" +
                  "InboundAdditionalDocuments" +
                  "/Forms/AllItems.aspx?FilterField1=TransmittalID&FilterValue1=" +
                  inboundTransmittalHeader.data.ID +
                  "&FilterType1=Lookup&viewid=d22d3ef1%2Dca95%2D4a3c%2Da124%2Dbeb594f07906",
              },
            };
              ...
              for (let i = 0; i < this.state.ReactTableResult.length; i++) {
                let inboundDetails = {
                  ...
                  CustomerDocumentNumber:
                    this.state.ReactTableResult[i].companyDocNumber,
                  PONumber: this.state.ReactTableResult[i].poNumber,
                  TransmittalCodeId:
                    this.state.ReactTableResult[i].transCodeKey,
                  TransmittalHeaderId: this.state.inboundTransmittalHeaderId,
                };
                await this._Service
                  .createNewItem(
                    this.props.siteUrl,
                    this.props.InboundTransmittalDetails,
                    inboundDetails
                  )
                  .then(async (iar) => {
                    ...
                    let updateIndex = {
                      ...
                      CurrentStatusCode:
                        this.state.ReactTableResult[i].transmittalCode,
                      CurrentReplyDate:
                        this.state.ReactTableResult[i].receiveDate,
                      CustomerDocumentNo:
                        this.state.ReactTableResult[i].companyDocNumber,
                    };
                    await this._Service
                      .updateItem(
                        ...
                        this.props.documentIndexList,
                        updateIndex,
                        parseInt(this.state.ReactTableResult[i].docId)
                      )
                      .then((iar) => {
                        this._Service
                          .getItemById(
                            this.props.siteUrl,
                            this.props.documentIndexList,
                            this.state.ReactTableResult[i].docId
                          )
                          .then(async (iar) => {
                            console.log(iar);
                            this.setState({
                              SourceDocumentID: iar.SourceDocumentID,
                            });
                            this.setState({ DocumentID: iar.DocumentID });
                            let select =
                              "ID,TransmittalHeader/ID,DocumentIndex/ID,DocumentIndex/Title";
                            let filter =
                              "DocumentIndex/ID eq '" +
                              Number(this.state.ReactTableResult[i].docId) +
                              "'";
                            let expand = "DocumentIndex,TransmittalHeader";
                            await this._Service
                              .getItemWithFilterExpand(
                                this.props.siteUrl,
                                this.props.OutboundTransmittalDetails,
                                select,
                                filter,
                                expand
                              )
                              .then(
                                async (outboundTransmittalDetailsListNameh) => {
                                  if (
                                    outboundTransmittalDetailsListNameh.length >
                                    0
                                  ) {
                                    for (
                                      let k = 0;
                                      ...
                                      this.setState({
                                        TransmittalHeaderId:
                                          outboundTransmittalDetailsListNameh[k]
                                            .TransmittalHeader.ID,
                                      });
                                      let ouboundUpdate = {
                                        TransmittalStatus: "Completed",
                                        ReceivedDate:
                                          this.state.ReactTableResult[i]
                                            .receiveDate,
                                        ReceivedComments:
                                          this.state.ReactTableResult[i]
                                            .comments,
                                  ...
                                  await this._Service.createNewItem(
                                    this.props.siteUrl,
                                    this.props.TransmittalHistory,
                                    transmittalHistory
                                  );
                                  let select =
                                    "SourceDocumentID,Owner/ID,Owner/Title,Owner/EMail,DocumentName,Approver/ID,Approver/Title,Approver/EMail,Reviewers/ID,Reviewers/Title,Reviewers/EMail";
                                  ...
                                  this._Service
                                    ...
                                    .then((forGettingOwner) => {
                                      this._sendmail(
                                        forGettingOwner[0].DocumentName,
                                        forGettingOwner[0].Owner.EMail,
                                        "InboundTransmittalFromCustomer",
                                        forGettingOwner[0].Owner.Title,
                                        this.state.ReactTableResult[i]
                                          .transmittalCode
                                      );
                                      this._sendmail(
                                        forGettingOwner[0].DocumentName,
                                        forGettingOwner[0].Approver.EMail,
                                        "InboundTransmittalFromCustomer",
                                        forGettingOwner[0].Approver.Title,
                                        this.state.ReactTableResult[i]
                                          .transmittalCode
                                      );
                                      if (forGettingOwner[0].Reviewers) {
                                        for (let k in forGettingOwner[0]
                                          .Reviewers) {
                                          if (
                                            forGettingOwner[0].Reviewers[k]
                                              .EMail !=
                                              forGettingOwner[0].Owner.EMail ||
                                            forGettingOwner[0].Reviewers[k]
                                              .EMail !=
                                      ...
                                      this._triggerPermission(
                                        forGettingOwner[0].SourceDocumentID
                                      );
                                    });
                                  let selectHeaderItems =
                                    "TransmittalHeader/ID,TransmittalStatus";
                                  let filter =
                                    "TransmittalHeader/ID eq '" +
                                    Number(this.state.TransmittalHeaderId) +
                                    "' ";
                                  ...
                                  this._Service
                                    ...
                                    .then(
                                      (outboundTransmittalDetailsListName) => {
                                        let length =
                                          outboundTransmittalDetailsListName.length;
                                        ...
                                        if (length == recievedlength) {
                                          ...
                                          this._Service.updateItem(
                                            this.props.siteUrl,
                                            this.props
                                              .OutboundTransmittalHeader,
                                            updateTransmittalStatus,
                                            parseInt(
                                              this.state.TransmittalHeaderId
                                            )
                                          );
                                        }
                                      }
              ...
              for (let i = 0; i < this.state.ReactTableResult2.length; i++) {
                await this._Service
                  .uploadDocument(
                    this.props.siteUrl +
                      "/" +
                      this.props.InboundAdditionalDocuments,
                    this.state.transmittalID +
                      this.state.ReactTableResult2[i].Attachments2.name,
                    this.state.ReactTableResult2[i].Attachments2
                  )
                  .then(async (f) => {
                    ...
                    await f.file.getItem().then(async (item) => {
                      await item.update({
                        ...
                        ReceivedDate:
                          this.state.ReactTableResult2[i].receiveDate2,
        ...
        this._Service
          ...
          .then(async (inboundTransmittalHeader) => {
            ...
            let selectHeaderItems =
              "ID,Title,Attachments,AttachmentFiles,DocumentIndex/ID,DocumentIndex/Title,DocumentIndex/DocumentName,Comments,Owner/ID,Owner/Title,TransmittalHeader/ID,ReceivedDate,CustomerDocumentNumber,Comments,TransmittalCode/Title,TransmittalCode/ID,PONumber";
            let expandItems =
              "AttachmentFiles,DocumentIndex,Owner,TransmittalHeader,TransmittalCode";
            let filter =
              "TransmittalHeader/ID eq '" + Number(this.transmittalID) + "' ";
            this._Service
              .getItemWithFilterExpand(
                this.props.siteUrl,
                this.props.InboundTransmittalDetails,
            ...
            if (
              this.state.ReactTableResult.length >
              this.state.currentinBoundDetailItem.length
            ) {
              for (
                let i = this.state.currentinBoundDetailItem.length;
                i < this.state.ReactTableResult.length;
                i++
              ) {
                let inboundDetails = {
                  Title: this.state.ReactTableResult[i].transmittalID,
                      ...
                      .then(async (di) => {
                        ...
                        await this._Service
                          ...
                          .then(async (outboundTransmittalDetailsListNameh) => {
                            if (
                              outboundTransmittalDetailsListNameh.length > 0
                            ) {
                              for (
                                let k = 0;
                                k < outboundTransmittalDetailsListNameh.length;
                                k++
                              ) {
                                this.setState({
                                  TransmittalHeaderId:
                                ...
                                let obDetailItems = {
                                  ...
                                  ReceivedDate:
                                    this.state.ReactTableResult[i].receiveDate,
                                  ReceivedComments:
                                    this.state.ReactTableResult[i].comments,
                                };
                                this._Service.updateItem(
                                  this.props.siteUrl,
                                  this.props.OutboundTransmittalDetails,
                                  obDetailItems,
                                  parseInt(
                                    outboundTransmittalDetailsListNameh[k].ID
                                  )
                                );
                              }
                            }
                              ...
                              .then((forGettingOwner) => {
                                ...
                                if (forGettingOwner[0].Reviewers) {
                                  for (let k in forGettingOwner[0].Reviewers) {
                                    if (
                                      forGettingOwner[0].Reviewers[k].EMail !=
                                        forGettingOwner[0].Owner.EMail ||
                                      forGettingOwner[0].Reviewers[k].EMail !=
                                        forGettingOwner[0].Approver.EMail
                                    ) {
                                      this._sendmail(
                                        ...
                                        forGettingOwner[0].Reviewers[k].EMail,
                                        "InboundTransmittalFromCustomer",
                                        forGettingOwner[0].Reviewers[k].Title,
                                        this.state.ReactTableResult[i]
                                          .transmittalCode
                                      );
                                    }
                            ...
                            this._Service
                              ...
                              .then((outboundTransmittalDetailsListName) => {
                                ...
                                let recievedlength = 0;
                                if (
                                  outboundTransmittalDetailsListName.length > 0
                                ) {
                                  for (
                                    let k = 0;
                                    k <
                                    ...
                                    if (
                                      outboundTransmittalDetailsListName[k]
                                        .TransmittalStatus == "Completed"
                                    ) {
                                ...
                                if (length == recievedlength) {
                                  ...
                                  this._Service.updateItem(
                                    this.props.siteUrl,
                                    this.props.OutboundTransmittalHeader,
                                    updateTS,
                                    parseInt(this.state.TransmittalHeaderId)
                                  );
                                }
                              }); //last
                          });
            ...
            await this._Service
              .getItemWithFilterDL(
                this.props.siteUrl,
                this.props.InboundAdditionalDocuments,
                "TransmittalIDId eq '" + this.transmittalID + "' "
              )
              .then((inboundAdditionalDocumentsListName) => {
                this.setState({
                  currentInboundAdditionalItem:
                    inboundAdditionalDocumentsListName,
                });
              });
            let i;
            if (
              this.state.ReactTableResult2.length >
              this.state.currentInboundAdditionalItem.length
            ) {
              for (
                i = this.state.currentInboundAdditionalItem.length;
                i < this.state.ReactTableResult2.length;
                i++
              ) {
                await this._Service
                  .uploadDocument(
  ...
  private bindInboundTransmittalSavedData(transmittalID) {
    try {
          ...
          if (inboundTransmittalDetailsListName.length > 0) {
            for (let k = 0; k < inboundTransmittalDetailsListName.length; k++) {
              ...
              data = {
                ...
                documentIndex:
                  inboundTransmittalDetailsListName[k].DocumentIndex[
                    "DocumentName"
                  ],
                companyDocNumber:
                  inboundTransmittalDetailsListName[k].CustomerDocumentNumber,
                receivedDate:
                  inboundTransmittalDetailsListName[k].ReceivedDate == null
                    ? null
                    : moment(
                        inboundTransmittalDetailsListName[k].ReceivedDate
                      ).format("DD/MM/YYYY"),
                receiveDate: inboundTransmittalDetailsListName[k].ReceivedDate,
                transmittalCode:
                  inboundTransmittalDetailsListName[k].TransmittalCode[
                    "Title"
                  ] == null
                    ? null
                    : inboundTransmittalDetailsListName[k].TransmittalCode[
                ...
                transCodeKey:
                  inboundTransmittalDetailsListName[k].TransmittalCode["ID"] ==
                  null
                    ? null
                    : inboundTransmittalDetailsListName[k].TransmittalCode[
                        "ID"
                ...
                docKey:
                  inboundTransmittalDetailsListName[k].DocumentIndex["Title"],
                Attachments:
                  inboundTransmittalDetailsListName[k].AttachmentFiles[0] ==
                  null
                    ? null
                    : inboundTransmittalDetailsListName[k].AttachmentFiles[0],
                ss:
      ...
      let tableData2 = this.state.ReactTableResult2;
      let data2: ITableData2;
      const h = this._Service.getListItems(
        this.props.siteUrl,
        this.props.InboundAdditionalDocuments
      );
      console.log(h);
      this._Service
        ...
        .then((inboundAdditionalDocumentsListName) => {
          ...
          if (inboundAdditionalDocumentsListName.length > 0) {
            for (
              let k = 0;
              k < inboundAdditionalDocumentsListName.length;
              k++
            ) {
              data2 = {
                Attachments2: null,
                ...
                adddocurl:
                  inboundAdditionalDocumentsListName[k]
                    .ServerRedirectedEmbedUrl,
                receivedDate2:
                  inboundAdditionalDocumentsListName[k].ReceivedDate == null
                    ? null
                    : moment(
                        inboundAdditionalDocumentsListName[k].ReceivedDate
                      ).format("DD/MM/YYYY"),
  ...
  public async saveAsDraft() {
    if (this.state.ReactTableResult.length == 0) {
      ...
      if (this.transmittalID == null || this.transmittalID == "") {
        ...
        const inboundTransmittalHeader = await this._Service.createNewItem(
          this.props.siteUrl,
          this.props.InboundTransmittalHeader,
          inboundHeader
        );
        if (inboundTransmittalHeader) {
          ...
          const updateTransmittalHeader = await this._Service.updateItem(
            this.props.siteUrl,
            this.props.InboundTransmittalHeader,
            inboundLinks,
            inboundTransmittalHeader.data.ID
          ...
          if (updateTransmittalHeader) {
            if (this.state.ReactTableResult.length > 0) {
              for (let i in this.state.ReactTableResult) {
                try {
                  ...
                  const inboundTransmittalDetails =
                    await this._Service.createNewItem(
                      this.props.siteUrl,
                      this.props.InboundTransmittalDetails,
                      inboundDetails
            ...
            if (this.state.ReactTableResult2.length > 0) {
              for (let i in this.state.ReactTableResult2) {
                const inboundadditionaldocuments =
        ...
        const inboundTransmittalHeader = await this._Service.updateItem(
          this.props.siteUrl,
          this.props.InboundTransmittalHeader,
          inboundHeader,
          this.transmittalID
      ...
      this.setState({
        ...
        statusMessage: {
          isShowMessage: true,
          message: "Transmittal Saved as Draft",
          messageType: 4,
        },
      });
      setTimeout(() => {
  ...
  public _projectInformation = async () => {
    const projectInformation = await this._Service.getListItems(
      this.props.siteUrl,
  ...
  public async _bindData() {
    ...
    let filter =
      "TransmittalStatus eq 'Ongoing' and (WorkflowStatus eq 'Published')";
    const documentIndex: any[] = await this._Service.getItemWithSelectFilter(
      this.props.siteUrl,
      this.props.documentIndexList,
      select,
      filter
    ...
    const TransmittalCodeSettings: any[] = await this._Service.getListItems(
      this.props.siteUrl,
      this.props.TransmittalCodeSettings
    );
  ...
  public render(): React.ReactElement<IInboundCustomerProps> {
    ...
    return (
      <div>
        ...
        <div
          className={styles.InboundCustomerWp}
          style={{ display: this.state.webpartView }}
        >
          <div>
            <div className={styles.documenttitle}>
              Inbound Transmittal from {this.state.outlookCustomer}
            </div>
            <div hidden={this.state.transIdvisible}>
              <div
                style={{ display: "flex", margin: "7px" }}
                hidden={this.state.transIdvisible}
              >
                <Label hidden={this.state.transIdvisible}>
                  Transmittal ID : {this.state.transmittalID}
                </Label>
              </div>
            </div>
            <div className={styles.row}>
              <div style={{ display: "flex", margin: "7px" }}>
                ...
                <Label style={{ padding: "0 0 0 185px" }}>
                  Project : {this.state.projectNumber}-{this.state.projectName}
                </Label>
              </div>
              <div className={styles.border}>
                ...
                <div style={{ display: "flex", margin: "7px" }}>
                  ...
                  <input
                    type="file"
                    style={{ margin: "7px", paddingLeft: "7px" }}
                ...
                <div style={{ display: "flex", width: "100%", margin: "7px" }}>
                  <div style={{ width: "50%", marginRight: "4%" }}>
                    <TextField
                      ...
                      id="ponumber"
                      name="ponumber"
                      value={this.state.outlookPONumber}
                      onChange={this._PoNumbChange}
                      style={{ width: "100%" }}
                      readOnly={true}
                    ></TextField>
                    <div style={{ color: "#dc3545" }}>
                  ...
                  <div style={{ width: "50%" }}>
                    <TextField
                      id="CompanyDocNo"
                      name="CompanyDocNo"
                      label="Customer Doc No"
                      onChange={this._CompanyDocChange}
                      value={this.state.outlookCustomerDocNo}
                      style={{ width: "100%" }}
                    ></TextField>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", margin: "7px", width: "100%" }}>
                  ...
                  <div>
                    <IconButton
                      ...
                      style={{ padding: "58px 0px 0px 45px" }}
                    />
                  </div>
                </div>
                <div hidden={this.state.docselected}>
                  {" "}
                  <div style={{ color: "#dc3545" }}>
                ...
                <div hidden={this.state.AddIndex}>
                  ...
                  >
                    <tr>
                      ...
                      <th
                        hidden={this.state.queryParamYes}
                        style={{
                          border: "1px solid #ddd",
                          padding: "8px",
                          borderCollapse: "collapse",
                      ...
                      <th
                        hidden={this.state.queryParamNo}
                        style={{
                          border: "1px solid #ddd",
                          padding: "8px",
                          borderCollapse: "collapse",
                      ...
                      <th
                        hidden={this.state.projectdivVisible}
                        style={{
                          border: "1px solid #ddd",
                          padding: "8px",
                          borderCollapse: "collapse",
                ...
                <div>
                  ...
                  <div hidden={this.state.docaddselected}>
                    {" "}
                    <div style={{ color: "#dc3545" }}>
                      <Label style={{ color: "#dc3545" }}>
                        Document already selected.{" "}
                  ...
                  <div
                    style={{ width: "100%", display: "flex", margin: "7px" }}
                  >
                    <div style={{ width: "50%" }}>
                      ...
                      <input
                        ...
                        id="additionalfile"
                        ref={this.fileInput}
                        onChange={this.handleFileUpload2}
                      ></input>
                    </div>
                    <div style={{ width: "50%" }}>
                      <DatePicker
                  ...
                  <div
                    ...
                    style={{ display: "flex", margin: "7px", width: "100%" }}
                  >
                    <div style={{ width: "90%" }}>
                      <TextField
                        label="Comments"
                        ...
                        {this.state.ReactTableResult2.map((item, key) => {
                          return (
                            <tr key={key}>
                              ...
                              >
                                <a target="_blank" href={item.adddocurl}>
                                  {item.documentName2}
                                </a>
                              </td>
                              <td
              ...
              <div
                style={{
                  display: "flex",
                  padding: "33px 26px 12px 2px",
                  float: "right",
                }}
                hidden={this.state.btnsvisible}
              >
