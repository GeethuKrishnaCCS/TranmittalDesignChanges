...
import { ActionButton, Checkbox, ChoiceGroup, DatePicker, DefaultButton, Dialog, DialogFooter, DialogType, Dropdown, IChoiceGroupOption, IChoiceGroupStyles, IDropdownOption, IIconProps, ITooltipHostStyles, IconButton, Label, MessageBar, Modal, PrimaryButton, Spinner, TextField, TooltipHost, getTheme, mergeStyleSets } from '@fluentui/react';
...
export default class TransmittalCreateDocument extends React.Component<ITransmittalCreateDocumentProps, ITransmittalCreateDocumentState, {}> {
  ...
  private newheaderid: number;
  private postUrlForUnderReview: any;
  private postUrlForAdaptive: any;
  ...
  public async _onCreateDocument() {
    if (this.state.createDocument === true && this.isDocument === "Yes" || this.state.createDocument === false) {
      if (this.state.expiryDate !== null || this.state.expiryDate !== undefined) {
        if (this.props.project) {
          console.log(this.state.dccEmail);
          ...
          //Validation with direct publish
            ...
            && this.validator.fieldValid('Approver') && this.validator.fieldValid('subcategory')
          ...
          //Validation without direct publish
          if (this.validator.fieldValid('Title') && this.validator.fieldValid('subcategory') && this.validator.fieldValid('category') && this.validator.fieldValid('BU/Dep') && this.validator.fieldValid('Owner') && this.validator.fieldValid('Approver') && this.validator.fieldValid('legalentity')
            // && this.validator.fieldValid('expiryDate') && this.validator.fieldValid('ExpiryLeadPeriod')
          ) {
            this.setState({
              saveDisable: true, hideCreateLoading: " ",
          ...
          //Validation with direct publish
          else if (this.validator.fieldValid('Title') && this.validator.fieldValid('subcategory') && this.validator.fieldValid('category') && this.validator.fieldValid('BU/Dep') && (this.state.directPublishCheck === true) && this.validator.fieldValid('publish') && this.validator.fieldValid('Owner') && this.validator.fieldValid('Approver') && this.validator.fieldValid('legalentity')
            //  && this.validator.fieldValid('expiryDate') && this.validator.fieldValid('ExpiryLeadPeriod')
          ) {
            this.setState({
              saveDisable: true, hideloader: false, hideCreateLoading: " ",
        ...
        if (this.props.project) {
          //Validation without direct publish
          if (this.validator.fieldValid('Title') && this.validator.fieldValid('subcategory') && this.validator.fieldValid('category') && this.validator.fieldValid('BU/Dep') && this.validator.fieldValid('Owner')
          ...
          //Validation with direct publish
          else if (this.validator.fieldValid('Title') && this.validator.fieldValid('subcategory') && this.validator.fieldValid('category') && this.validator.fieldValid('BU/Dep')
          ...
          //Validation with direct publish
          else if (this.validator.fieldValid('Title') && this.validator.fieldValid('category') && this.validator.fieldValid('subcategory')
      ...
      if ((document.querySelector(upload) as HTMLInputElement).files[0] != null) {
            ...
            if (item) {
              ...
              if (this.state.directPublishCheck === false) {
                ...
                if (this.state.sendForReview === true) {
                  // alert(this.state.sendForReview);
                  this._dccReview();
                }
              }
              else {
        ...
        if (this.state.sourceId === "QDMS") {
          await this._Service.getqdmsselectLibraryItems(this.props.QDMSUrl, this.props.publisheddocumentLibrary).then((publishdoc: any) => {
            ...
            await this._Service.getBuffer(qdmsURl)
  ...
  private _LAUrlGettingForUnderReview = async () => {
    const laUrl = await this._Service.gethubListItems(this.props.hubUrl, this.props.requestList);
    console.log("Posturl", laUrl[0].PostUrl);
  ...
  public _adaptiveCard = async (Workflow: any, Email: any, Name: any, Type: any, detailId: any, Taskid: any) => {
    let siteUrl = window.location.protocol + "//" + window.location.hostname + this.props.siteUrl;
    ...
    const body: string = JSON.stringify({
      ...
      'SourceDocumentID': String(this.state.sourceDocumentId),
  ...
  public async _dccReview() {
    const headerdata = {
      ...
      ApproverId: this.state.approver,
      ReviewersId: this.state.reviewers,
      RequesterId: this.state.owner,
      RequesterComment: this.state.comments,
      RequestedDate: new Date(),
      Workflow: "DCC Review",
      DocumentControllerId: parseInt(this.state.dcc),
      RevisionCodingId: this.state.revisionCodingId,
      OwnerId: this.state.owner,
      PreviousReviewHeader: "0",
    }
    const header = await this._Service.createNewItem(this.props.siteUrl, "WorkflowHeader", headerdata);
    if (header) {
      ...
      if (log) {
        ...
        const detail = await this._Service.createNewItem(this.props.siteUrl, "WorkflowDetails", detaildata2);
        if (detail) {
          const user = await this._Service.getByhubEmail(this.state.dccEmail);
          console.log('User Id: ', user.Id);
          if (user) {
            // my task updation
            const taskdata5 = {
              Title: "Document Controller Review '" + this.state.documentName + "'",
              Description: "DCC Review request for  '" + this.state.documentName + "' by '" + this.state.currentUser + "'",
              DueDate: this.state.DueDate,
              StartDate: new Date(),
              ...
              Link: {
                ...
                Url: this.props.siteUrl + "/SitePages/DocumentReview.aspx?hid=" + this.newheaderid + "&dtlid=" + detail.data.ID + "&wf=dcc"
              },

            }
            const task = await this._Service.createhubNewItem(this.props.hubUrl, "WorkflowTasks", taskdata5);
            if (task) {
              const taskdetail = {
                Link: {
                  Description: this.state.documentName + "-Review",
              ...
              await this._Service.updateItem(this.props.siteUrl, "WorkflowDetails", taskdetail, detail.data.ID);
              //notification preference checking                                

              this._sendmail(this.state.dccEmail, "DocDCCReview", this.state.dccName, detail.data.ID);
              this._adaptiveCard("DCC Review", this.state.dccEmail, this.state.dccName, "Project", detail.data.ID, task.data.ID)
            }
          }
  ...
  //Send Mail
  public _sendmail = async (emailuser: any, type: any, name: any, detailId: any) => {
    console.log(name);
    //let mailSend = "Yes";
    let Subject;
    let Body;
    ...
    if (type === "DocDCCReview") {
      link = `<a href=${window.location.protocol + "//" + window.location.hostname + this.props.siteUrl + "/SitePages/DocumentReview.aspx?hid=" + this.newheaderid + "&dtlid=" + detailId + "&wf=dcc"} >Link</a>`;
    }
  ...
  private _closeModal() {
    this.setState({ showReviewModal: false, DueDate: null, comments: "" });
  }
  ...
  public async _publishUpdate() {
    ...
    //let updateindex ;
    await this._Service.updateItem(this.props.siteUrl, this.props.documentIndexList, updateindexdata, this.documentIndexID)
        ...
        <div style={{ display: this.state.createDocumentProject }} >
          <div className={styles.border}>
            ...
            <div className={styles.divrow}>
              ...
              <div className={styles.wdthThirdColm}>
            ...
            <div className={styles.divrow}>
              <div className={styles.wdthfrst}>
                <PeoplePicker
                  ...
                  resolveDelay={1000} />
                <div style={{ color: "#dc3545" }}>
                  {this.validator.message("DocumentController", this.state.dccEmail, "required")}{" "}</div>
              </div>
              <div className={styles.wdthmid}>
                <PeoplePicker
                  context={this.props.context as any}
            ...
            <DialogFooter>
              <div className={styles.rgtalign} >
                {this.state.saveDisable === false &&
                  <PrimaryButton id="b2" className={styles.btn} disabled={(this.state.dccEmail === "" || this.isDocument !== "Yes")} onClick={this._onSendForReview} >Submit and Send For Review </PrimaryButton >
                }
